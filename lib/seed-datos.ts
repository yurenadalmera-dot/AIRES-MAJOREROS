...
//
// Datos de demostración y cuenta de administración.
//
// Vive en `lib/` y no en `prisma/` porque lo llaman dos sitios: el script de
// línea de órdenes (`prisma/seed.ts`, `npm run db:seed`) y el arranque del
// servidor (`instrumentation.ts`), que es quien lo ejecuta en producción —
// allí es el único proceso con acceso a la base de datos.
 ...
 *
 * BORRA todo antes de sembrar, así que no puede correr a ciegas: con datos
 * existentes no hace nada y sale bien, para no llevarse por delante lo que
 * haya introducido la clienta. `forzar` es lo que permite reconstruir a
 * propósito.
 */
export async function sembrar({ forzar = false }: { forzar?: boolean } = {}) {
