...
async function viviendaDeLaOrganizacion(propertyId: string, organizationId: string) {
  ...
  if (!property) throw new ErrorDeNegocio("La vivienda indicada no existe");
  return property;
}
...
export async function createBooking(formData: FormData) {
  return conErroresLegibles(async () => {
    ...
    if (checkOut <= checkIn) {
      throw new ErrorDeNegocio("La fecha de salida debe ser posterior a la de entrada");
    }
...
export async function updateBooking(bookingId: string, formData: FormData) {
  return conErroresLegibles(async () => {
    ...
    if (!existing) throw new ErrorDeNegocio("Reserva no encontrada");
