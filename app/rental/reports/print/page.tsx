...
export default async function OwnerReportPrintPage({
  searchParams,
}: {
  ...
  const periodStart = new Date(params.start);
  const periodEnd = new Date(params.end);
  periodEnd.setHours(23, 59, 59, 999);

  const [owner, rentalBusiness] = await Promise.all([
    prisma.owner.findFirst({
      where: { id: params.ownerId, organizationId },
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="flex justify-between items-center mb-4 no-print">
        <BackButton href="/rental/reports" />
        <PrintButton />
      </div>
